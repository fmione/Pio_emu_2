...
1. `start` initializes the experiment in SQLite, creates initial state files, then enters a loop:
   - Compute simulated time step (`wall_clock × acceleration`)
   - Run ODE integration (Monod-type bioreactor model)
   - Sample noisy measurements
   - Publish to MQTT (OD readings, glucose, feed events)
...
**Configuration:** Edit `emulator/configs/EMULATOR_config.json`:
